---
layout: post
title: Battle-tested - the new north star for quality
---

POCs mean nothing to me anymore.

<!--more-->

The other day, I was thinking about how I pick my stack.

“Built” means nothing to me anymore. Agents can produce the most sophisticated code — well-structured, true rocket science. They can build almost anything without breaking a sweat.

I choose battle-tested implementations. I let time decide which implementations win and which perish. Those that survive must have gone through the brutal cycles of reality: functional issues, security issues, performance issues. Even the most tasteless software, when exposed to enough user feedback, eventually ends up in a slick state.

I choose mature technologies. Agents can implement anything. The value lies in technologies that actually solve real problems — that have found product-market fit.

The next time you want to add that dependency with 5 stars, how about you let your agent vibe-copy it instead?